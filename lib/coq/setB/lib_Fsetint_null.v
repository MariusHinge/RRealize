...
Axiom min_elt_def1 : forall (s:set Z), ~ (is_empty s) -> mem (min_elt s) s.
...
Axiom max_elt_def1 : forall (s:set Z), ~ (is_empty s) -> mem (max_elt s) s.
...
Axiom below_def :
  forall (x:Z) (n:Z), (mem x (below n)) <-> ((0%Z <= x)%Z /\ (x < n)%Z).

Axiom cardinal_below :
  forall (n:Z),
  ((0%Z <= n)%Z -> ((cardinal (below n)) = n)) /\
  (~ (0%Z <= n)%Z -> ((cardinal (below n)) = 0%Z)).
