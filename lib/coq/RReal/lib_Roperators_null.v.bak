...
Axiom Inverse_def :
  ...
  (set.Set.mem (y, x) (inverse1 r)) <-> (set.Set.mem (x, y) r).
...
Axiom injection :
  ...
  (set.Set.mem (inverse1 f) (infix_plmngt t s)) -> (set.Set.mem x s) ->
...
Axiom break_power_times :
  ...
  (set.Set.mem r (power (times1 u v))) <-> (set.Set.subset r (times1 u v)).

Axiom subset_of_times :
  forall {a:Type} {a_WT:WhyType a} {b:Type} {b_WT:WhyType b},
  forall (r:(a* b)%type -> bool) (u:a -> bool) (v:b -> bool),
  (set.Set.subset r (times1 u v)) <->
  forall (x:a) (y:b), (set.Set.mem (x, y) r) ->
  (set.Set.mem x u) /\ (set.Set.mem y v).

Parameter plus: (rreal* rreal)%type -> rreal.

Parameter times: (rreal* rreal)%type -> rreal.

Parameter uminus: rreal -> rreal.

Parameter inverse: rreal -> rreal.
...
Axiom inverseDef :
  forall (x:rreal), ~ (x = rzero) -> ((times (x, inverse x)) = rone).
...
Axiom absMult :
  forall (x:rreal) (y:rreal), ((abs (times (x, y))) = (times (abs x, abs y))).
