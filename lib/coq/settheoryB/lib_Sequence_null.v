...
(* Why3 assumption *)
Definition seq_length {a:Type} {a_WT:WhyType a} (n:Z) (s:a -> bool) :
    ((Z* a)%type -> bool) -> bool :=
  infix_mnmngt (mk 1%Z n) s.

Parameter seq:
  forall {a:Type} {a_WT:WhyType a}, (a -> bool) ->
  ((Z* a)%type -> bool) -> bool.

Parameter size: forall {a:Type} {a_WT:WhyType a}, ((Z* a)%type -> bool) -> Z.

Axiom mem_seq :
  forall {a:Type} {a_WT:WhyType a},
  forall (s:a -> bool) (r:(Z* a)%type -> bool),
  (set.Set.mem r (seq s)) <->
  ((0%Z <= (size r))%Z /\ (set.Set.mem r (seq_length (size r) s))).

Axiom seq_as_total_function :
  forall {a:Type} {a_WT:WhyType a},
  forall (s:a -> bool) (r:(Z* a)%type -> bool), (set.Set.mem r (seq s)) ->
  set.Set.mem r (infix_mnmngt (mk 1%Z (size r)) s).

Axiom size_def :
  forall {a:Type} {a_WT:WhyType a},
  forall (n:Z) (r:(Z* a)%type -> bool),
  ((size r) = n) <-> exists s:a -> bool, set.Set.mem r (seq_length n s).
